...
def joinFactors(factors):
    ...
    # make sets to store Unconditioned variables and condition variables.
    unconditionalVariables = set()
    conditionalVariables = set()

    # Iterate over all the factors to get all variables that are 
    # unconditioned and conditioned Variables.
    for factor in factors:
        unconditionalVariables.update(factor.unconditionedVariables())
        conditionalVariables.update(factor.conditionedVariables())

    # Remove all unconditioned variables from conditioned
    conditionalVariables.difference_update(unconditionalVariables)
    ...
    # Takes in as input a set of unconditioned Variables, a set of 
    # conditioned Variables, and a variable Domains Dict as a mapping from 
    # variables to domains.
    # Initializes the probability entries of all possible assignmentDicts to be 0.0
    newFactor = Factor(unconditionalVariables, conditionalVariables, variableDomainsDict)

    # Update the probability entries of all possible assignmentDicts.
    ...
    # return the probability entries of all possible assignmentDicts.
    return newFactor
...
def normalize(factor):
    ...
    for unconditionedVariable in unconditionalVariables.copy():
        if len(variableDomainsDict[unconditionedVariable]) == 1:
            unconditionalVariables.remove(unconditionedVariable)
            conditionalVariables.add(unconditionedVariable)
