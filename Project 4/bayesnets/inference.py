...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # get all the factors in the bayes network.
        factors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        
        # iterate over all the variables to be eliminated.
        for var in eliminationOrder:
            # Perform the Join Operation
            factors, temp_factor = joinFactorsByVariable(factors, var)
            
            # Perform the Elimination Operation. 
            if len(temp_factor.unconditionedVariables()) > 1:
                factor = eliminate(temp_factor, var)
